...
class ResponseTemplates:
    ...
    def get_conversation_response(self, intent: str) -> str:
        templates = self.conversation_templates.get(intent, self.conversation_templates['off_topic'])
...
class EnhancedRakutenMobileFAQBot:
    ...
    async def process_question(self, user_query: str) -> str:
        try:
            ...
            if not intent_info['requires_search']:
                response = self.response_templates.get_conversation_response(intent_info['intent'])
    ...
    async def get_llm_response(self, prompt: str) -> str:
        try:
            ...
            response = self.llm_client.chat.completions.create(
                model="gpt-4o",
                messages=[
                    {"role": "system", "content": "You are a helpful assistant that provides information about Rakuten Mobile services in Japanese."},
    ...
    def format_response(self, response: str, relevant_info: List[Dict[str, Any]]) -> str:
